...
void FCA_print_state(long int State,const char *IP,const char *Domain,const char *Name,const char *OS,const char *Comment,const char *MasterIP)
{
	...
	if(!FCA_quiet && !(FCA_disp_off[0]=='f' && State==FFSS_STATE_OFF)
		&& ! (FCA_disp_quiet[0]=='f' && State==FFSS_STATE_QUIET) ) {
		if(!FCA_nb_states) {
...
void FCA_print_domains(const char **Domains, int NbDomains)
{
	...
	if(NbDomains>=0) {
		...
		for(id=0; id<=NbDomains; id++) {
			...
			if(id!=NbDomains)
				Pdoms=FCA_get_Domain(Domains[id]);
			else
				Pdoms=FCA_get_Domain("None");
			sav=FCA_Servers;
			FCA_Servers=doms;
			if(id!=NbDomains)
				doms=FCA_add_Domain(Domains[id]);
			else
...
void FCA_print_servers(const char *Domain, const int NbHost,SU_PList HostList)
{
	...
	while(Ptr != NULL) {
		...
		if(!FCA_quiet  && !(FCA_disp_off[1]=='f' && H->State==FFSS_STATE_OFF)
		 && ! (FCA_disp_quiet[0]=='f' && H->State==FFSS_STATE_QUIET)) {
		 	if(!nbdisp) {
	...
	if(NbShares) {
		...
		for(ish=0; ish<NbShares; ish++) {
			...
			if(! FCA_quiet) {
				FCA_tab_pre_item();
				 FCA_pre_tab_item();
				  FCA_pre_dir(FCA_tolist_dir, Names[ish], true);
				   FCA_tab_item(Names[ish],FFSS_MAX_SHARENAME_LENGTH);
				  FCA_post_dir(true);
				 FCA_post_tab_item();
				FCA_tab_int_item();
				 FCA_pre_tab_item();
				  FCA_tab_item(Comments[ish],0);
				 FCA_post_tab_item();
				FCA_tab_post_item();
			}
		}
